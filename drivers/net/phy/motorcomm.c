...
static int yt8521_probe(struct phy_device *phydev)
{
	...
	priv->fe_tx_delay = 0x0F;
	priv->ge_tx_delay = 0x01;
	priv->rx_delay = 0x0;
	if (of_node) {
		of_property_read_u16(of_node, "yt8521,fe-tx-delay", &priv->fe_tx_delay);
		of_property_read_u16(of_node, "yt8521,ge-tx-delay", &priv->ge_tx_delay);
		of_property_read_u16(of_node, "yt8521,rx-delay", &priv->rx_delay);
	}
...
static int yt8521_config_init(struct phy_device *phydev)
{
	...
	case PHY_INTERFACE_MODE_RGMII:
		...
		break;
	case PHY_INTERFACE_MODE_RGMII_RXID:
		val = (priv->ge_tx_delay << 0) | (priv->fe_tx_delay << 4);
		val |= (priv->rx_delay << 10);
		break;
	case PHY_INTERFACE_MODE_RGMII_TXID:
...
#if (LINUX_VERSION_CODE <= KERNEL_VERSION(5,19,17))
/**
